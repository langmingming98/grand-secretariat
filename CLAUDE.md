...
**React hooks ordering:** `useCallback` definitions that depend on values from custom hooks (like `useRoomSocket`) must be placed *after* the hook call. TypeScript catches this as "used before declaration".
...
Production uses nginx basic auth via `.htpasswd` file (mounted to `/etc/nginx/.htpasswd` in the nginx container).

```bash
# Reset password (generates new htpasswd entry)
htpasswd -nb username newpassword > .htpasswd

# Redeploy to apply changes
# (push to main triggers CI/CD, or manually restart nginx container)
```
